...
class MessageConsumeOauth:
    ...
    def consume_from_topic(self,topic,group,results):
                ...
                for msg in msgs:
                    if msg.error():
                        ...
                        if msg.error().code() == KafkaError._PARTITION_EOF:
                            logger.error(f"Consumed all the messages")
                        else:
    ...
    def secquence(self):
        ...
        for i in range(len(self.topic)):
            t = threading.Thread(target=self.consume_from_topic, args=(self.topic[i],self.group[i],results,))
            threads.append(t)
            t.start()
        ...
        print(f"All consumer results :: {results}")
        # Calculate the total sum of values from all threads
        percentile = np.percentile(results , 98)
        result={
                "status": f"sucessfully consumed {str(self.count)} messages across {str(len(self.topic))} topics",
